...
export const IngredientForm = ({ onIngredientsChange }: IngredientFormProps) => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        const response = await getRecipeUnits({ pagination: { page: 1, per_page: 50 } });
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-4 mb-2">
        <span className="flex-1 text-sm font-medium text-gray-700">Nome do Ingrediente</span>
        <span className="w-24 text-sm font-medium text-gray-700">Quantidade</span>
      ...
      {ingredients.map((ingredient, index) => (
        <div key={index} className="flex gap-4 items-center">
          ...
          <input
            ...
            onChange={(e) => handleChange(index, "quantity", e.target.valueAsNumber)}
          />
          <select
            className="w-32 p-2 border rounded shadow-sm"
