...
export default function RecipesHome() {
  ...
  const fetchCategories = async () => {
    try {
      const response: PaginationResponse<RecipeCategory> = await recipeCategoryService.getAll({
        page: 1, per_page: 100,
      });
  ...
  const fetchDiets = async () => {
    try {
      const response: PaginationResponse<RecipeDiet> = await recipeDietService.getAll({
        page: 1, per_page: 100,
      });
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl w-full space-y-8">
      <div className="px-4">
        ...
        <form onSubmit={handleSubmit} className="mb-6 flex gap-2">
          ...
          />
          <FilledButton text="Pesquisar" type="submit" />
        </form>
